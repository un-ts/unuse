...
export function useEventListener(
  ...
  const event = unAccess(firstParamTargets) ? args[1] : args[0];
  const listener = unAccess(firstParamTargets) ? args[2] : args[1];
  const options = unAccess(firstParamTargets) ? args[3] : args[2];
  ...
  const computedForWatch = unComputed(() => {
    return [
      unAccess(firstParamTargets)?.map((e) => unRefElement(e as never)) ??
        [defaultWindow].filter((e) => e != null),
      toArray(unAccess(event as any) as unknown as string[]),
      toArray(unAccess(listener, { unwrapFunction: false })),
      unAccess(options) as boolean | AddEventListenerOptions | undefined,
    ] as const;
  });
